...
export function AccordionPanel({
  ...
  return (
    <div
      ...
      className={
        variant === "primary"
          ? "rounded-[2rem] border border-white/10 bg-black/20 p-5"
          : "rounded-[1.5rem] border border-white/10 bg-black/20 p-4 sm:p-5"
      }
    >
      <button type="button" onClick={onToggle} className="flex w-full items-center justify-between gap-3 text-left">
        <span className={variant === "primary" ? "text-lg font-semibold text-white" : "text-sm font-semibold text-white"}>
          {title}
        </span>
