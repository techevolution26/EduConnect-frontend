  ...
  return (
    <RoleGuard allowedRoles={["WRITER", "TEACHER", "ADMIN"]}>
      <div className="mx-auto min-h-screen max-w-[1400px] overflow-x-hidden px-3 py-4 sm:px-6 lg:px-8">
            ...
            <div>
              ...
              <div className="mt-5 flex flex-wrap gap-2 sm:mt-6 sm:gap-3">
                {[
                  ...
                  <div key={stat.label} className="rounded-2xl border border-white/10 bg-white/[0.04] px-3 py-2 text-xs text-white/75 sm:px-4 sm:py-3 sm:text-sm">
                    <span className="text-white/45">{stat.label}</span>{" "}
                    <span className="ml-1 font-semibold text-white">{stat.value}</span>
                  </div>
                ))}
              </div>
            </div>
        ...
        {/* Form */}
        <form onSubmit={handleCreate} className="mt-5 grid w-full max-w-full gap-5 overflow-x-hidden 2xl:grid-cols-[1.35fr_0.65fr] sm:mt-6 sm:gap-6">
          {/* Main column */}
              ...
              >
                ...
                <div className="rounded-[1.5rem] border border-white/10 bg-white/[0.03] p-4">
                  ...
                  <p className="mt-2 break-words text-xs text-white/35">
                    Public URL preview: <span className="break-all text-white/55">/read/{slugField.finalSlug || "slug"}</span>
                  </p>
                </div>
          ...
          <aside className="min-w-0 space-y-5 2xl:sticky 2xl:top-6 2xl:self-start">
            ...
            <div className="rounded-[2rem] border border-white/10 bg-black/20 p-5">
              <div className="flex flex-col gap-3">
                <button
                  type="submit"
                  disabled={flow.create.isPending || flow.upload.isPending || Boolean(flow.createdContentId)}
