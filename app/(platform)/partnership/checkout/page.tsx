  ...
  if (startMutation.isSuccess) {
    return (
      <div className="flex min-h-[60vh] flex-col items-center justify-center gap-6 pb-10 text-center">
        <div className="flex h-16 w-16 items-center justify-center rounded-full border border-success/30 bg-success-soft">
        ...
        <div>
          ...
          <p className="mt-2 text-sm text-fg-dim">
            Check your phone at{" "}
            <span className="font-medium text-fg">{phoneNumber}</span> and enter
            your M-Pesa PIN to complete the payment.
          </p>
        </div>
        <Link
          href="/partnership"
          className="inline-flex items-center gap-2 rounded-2xl border border-border bg-surface px-5 py-3 text-sm font-semibold text-fg transition hover:bg-surface-2"
        >
    ...
    <div className="mx-auto max-w-lg space-y-6 pb-10">
      ...
      <section className="rounded-[2rem] border border-border bg-surface p-6">
        ...
        <p className="mt-1 text-xs text-fg-dim">
          An STK push will be sent to the number below. Enter your M-Pesa PIN to
          confirm.
        </p>

        <div className="mt-5">
          <label htmlFor="phone" className="block text-sm text-fg-dim">
            M-Pesa phone number
          </label>
          <div className="relative mt-2">
            <Phone className="pointer-events-none absolute left-4 top-1/2 h-4 w-4 -translate-y-1/2 text-fg-dim" />
            <input
              id="phone"
              type="tel"
              value={phoneNumber}
              ...
              autoComplete="tel"
              className={`w-full rounded-2xl border bg-surface py-3 pl-11 pr-4 text-sm text-fg outline-none transition placeholder:text-fg-dim/70 focus:border-accent/40 ${
                phoneError
        ...
        <button
          ...
          className="mt-6 inline-flex w-full items-center justify-center gap-2 rounded-2xl bg-accent px-4 py-3 text-sm font-semibold text-on-accent transition hover:opacity-90 disabled:cursor-not-allowed disabled:opacity-60"
        >
