...
function QuickLink({
  ...
  return (
    <Link
      href={href}
      className="group rounded-[1.5rem] border border-white/10 bg-white/[0.04] p-5 transition hover:border-white/20 hover:bg-white/[0.06]"
    >
    ...
    <section className="relative grid gap-8 lg:grid-cols-[1.25fr_0.75fr] lg:items-end">
      ...
      <div className="rounded-[2rem] border border-white/10 bg-black/25 p-5">
        ...
        <div className="mt-5 grid grid-cols-3 gap-3">
          {[
            { label: "Public", value: "Open", helper: "Discovery feed is public" },
            { label: "Premium", value: "Partner", helper: "Supports creator earnings" },
            { label: "Safety", value: "Curated", helper: "Moderated and structured" },
...
function AuthHero({
  ...
  return (
    <article className="relative overflow-hidden rounded-[2.5rem] border border-white/10 shadow-[0_24px_80px_rgba(0,0,0,0.42)]">
      {/* Background */}
      <div
...
function StickyFilterBar({
  ...
  return (
    <div className="sticky top-0 z-20 -mx-4 px-4 pb-2 pt-2 backdrop-blur-md sm:-mx-6 sm:px-6 lg:-mx-8 lg:px-8">
      <div className="flex flex-col gap-3 rounded-[2rem] border border-white/10 bg-[rgba(10,9,8,0.88)] p-3 shadow-[0_8px_32px_rgba(0,0,0,0.4)] md:flex-row md:items-center md:justify-between">
        {/* Tabs */}
        <div className="flex shrink-0 gap-2">
          {(["discover", "for-you"] as const).map((tab) => (
            <button
              ...
              className={`rounded-2xl px-4 py-2 text-sm font-medium transition ${activeTab === tab
                ? "bg-white text-black"
            ...
            {contentTypeFilters.map((filter) => {
              ...
              return (
                <button
                  ...
                  className={`shrink-0 rounded-2xl px-3.5 py-1.5 text-sm transition ${active
...
function FeaturedCard({ item }: { item: FeedItem }) {
  return (
    <article className="relative overflow-hidden rounded-[2.5rem] border border-white/10 bg-[#0f1117] p-6 shadow-[0_20px_80px_rgba(0,0,0,0.45)] md:p-8 lg:p-10">
      ...
      <div className="relative">
        <span className="inline-flex items-center gap-2 rounded-full border border-amber-400/20 bg-amber-500/10 px-4 py-2 text-xs font-medium uppercase tracking-[0.22em] text-amber-200">
          Featured story
        </span>
